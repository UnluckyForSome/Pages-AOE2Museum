...
function readSpriteFile(reader, limit) {
	...
	for (let i = 0; i < frameCount; ++i) {
		...
		if (frameType & 0x4) {
			...
			let unknown = reader.readChunk();
			adjustByUnknownLayer(unknown, frame);
		}
		if (frameType & 0x8) {
...
function adjustByUnknownLayer(rawData, frame) {
	let imageData = frame.data.normal;
	if (!imageData.inherited) {
		return;
	}
	let coord = frame.data.normal.coordinates;
	let width = coord[2] - coord[0];
	let height = coord[3] - coord[1];
	let stride = frame.data.normal.width;
	let rightLimit = coord[2];
	let data = imageData.data;

	let rows = height / 4;
	let start_offset = 2 + rows * 2;
	...
	let tile = 0;
	for (let p = 0; p < rows; ++p) {
		let off0 = offsets[p], off1 = offsets[p + 1];
		
		let xOff = coord[0], yOff = p * 4 + coord[1];
		let c = rawData[off0];
		if (c < 128) {
			xOff += c * 4;
			c = rawData[++off0];
