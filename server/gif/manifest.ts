// Build the /pages/gif/ dropdown manifests by intersecting the shipped mapping JSONs
...
export function loadSlpMapping(env: GifEnv): Promise<SlpMapping> {
  const m = memo();
  if (!m.slpMapping) {
    m.slpMapping = fetchAsset<SlpMapping>(env.ASSETS, "/modules/unit-gifs/slp_mapping.json");
  }
  return m.slpMapping;
}
...
export function loadSldMapping(env: GifEnv): Promise<SldMapping> {
  const m = memo();
  if (!m.sldMapping) {
    m.sldMapping = fetchAsset<SldMapping>(env.ASSETS, "/modules/unit-gifs/sld_mapping.json");
  }
  return m.sldMapping;
}
