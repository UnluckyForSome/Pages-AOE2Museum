...
impl SCXVersion {
    ...
    pub(crate) fn to_player_version(self) -> Option<f32> {
        match self.as_bytes() {
            ...
            // AoE2-era formats: these all use the same underlying "player_version" layout gates.
            // Accept newer DE2 container versions as well so we can parse the file and rely on
            // internal version markers (header/data/map/etc).
            b"1.20" | b"1.21" | b"1.32" | b"1.35" | b"1.36" | b"1.37" | b"1.57" => Some(1.14),
            _ => {
                // Generic acceptance for well-formed "1.xx" versions >= 1.20.
                let b = self.as_bytes();
                if b.len() == 4 && b[0] == b'1' && b[1] == b'.' && b[2].is_ascii_digit() && b[3].is_ascii_digit() {
                    let tens = (b[2] - b'0') as u16;
                    let ones = (b[3] - b'0') as u16;
                    let n = tens * 10 + ones;
                    if n >= 20 {
                        return Some(1.14);
                    }
