...
impl AIErrorInfo {
    ...
    pub fn read_from(mut input: impl Read) -> Result<Self> {
        ...
        let error_code_raw = input.read_u32::<LE>()?;
        let error_code = AIErrorCode::try_from(error_code_raw).ok();
    ...
    pub fn write_to(&self, mut output: impl Write) -> Result<()> {
        ...
        // Preserve raw code to avoid losing information for unknown variants.
        output.write_u32::<LE>(self.error_code_raw)?;
...
impl AIFile {
    ...
    pub fn read_from(mut input: impl Read) -> Result<Self> {
        // Some files in the wild contain empty optional strings here; treat them as empty
        // instead of panicking so higher-level parsing can continue.
        let filename = input.read_u32_length_prefixed_str()?.unwrap_or_default();
        let content = input.read_u32_length_prefixed_str()?.unwrap_or_default();
