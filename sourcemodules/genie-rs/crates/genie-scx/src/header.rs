...
impl DLCOptions {
    pub fn read_from(mut input: impl Read) -> Result<Self> {
        ...
        dependencies.reserve(num_dependencies as usize);
        for _ in 0..num_dependencies {
            let raw = input.read_i32::<LE>()?;
            // Some files contain unknown / sentinel DLC IDs; treat them as "optional unknown"
            // so parsing remains robust.
            if let Ok(dlc) = DLCPackage::try_from(raw) {
                dependencies.push(dlc);
            }
        }
